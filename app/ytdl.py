    ...
    async def __add_entry(self, entry, quality, format, folder, custom_name_prefix, playlist_strict_mode, playlist_item_limit, auto_start, bypass_archive, already):
        ...
        if etype.startswith('url'):
            ...
            if not self.queue.exists(entry['id']):
                dl = DownloadInfo(entry['id'], entry.get('title') or entry['id'], entry.get('webpage_url') or entry['url'], entry['extractor_key'].lower(), quality, format, folder, custom_name_prefix, error)
